...
def view_class(class_id):
    ...
    # Récupérer tous les élèves (User)
    students = [enrollment.enrolled_student for enrollment in enrolled_students]

    # Récupérer toutes les soumissions des élèves pour les exercices de la classe
    from models import ExerciseSubmission
    submissions = ExerciseSubmission.query \
        .filter(ExerciseSubmission.exercise_id.in_([ex.id for ex in exercises])) \
        .all()

    # Construire un dictionnaire: {student_id: {exercise_id: score}}
    student_scores = {student.id: {} for student in students}
    for sub in submissions:
        if sub.student_id in student_scores:
            student_scores[sub.student_id][sub.exercise_id] = sub.score

    # Calculer la moyenne par élève
    student_averages = {}
    for student in students:
        scores = [student_scores[student.id].get(ex.id) for ex in exercises if student_scores[student.id].get(ex.id) is not None]
        if scores:
    ...
    # Construction de avg_scores_list (moyenne par exercice)
    avg_scores_list = []
    for exercise in exercises:
        scores = [student_scores[student.id].get(exercise.id) for student in students if student_scores[student.id].get(exercise.id) is not None]
        if scores:
            avg_scores_list.append(round(sum(scores)/len(scores), 1))
        else:
            avg_scores_list.append(None)

    # Construction de student_averages_list (moyenne par élève)
    student_averages_list = []
    for student in students:
        avg = student_averages.get(student.id)
        student_averages_list.append(avg if avg is not None else None)
...
def export_class_excel(class_id):
    ...
    for student in students:
        row = {
            'Élève': student.username
        }
        for ex in exercises:
            row[ex.title] = student_scores[student.id].get(ex.id)
        row['Moyenne'] = round(student_averages[student.id], 2) if student_averages[student.id] is not None else None
        data.append(row)
    ...
    output.seek(0)
    filename = f"classe_{class_id}_notes.xlsx"
    return send_file(output, download_name=filename, as_attachment=True, mimetype='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')

# --- EXPORT PDF ---
from fpdf import FPDF
@app.route('/classes/<int:class_id>/export/pdf')
@login_required
def export_class_pdf(class_id):
    ...
    output.seek(0)
    filename = f"classe_{class_id}_notes.pdf"
